...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const checkUser = () => {
      if (!user?.id) {
        ...
        setLoading(false); // Only proceed if user is valid
      }
    };
    const timer = setTimeout(checkUser, 0); // Small delay to allow context to propagate
    return () => clearTimeout(timer);
  }, [user, navigate]);

  useEffect(() => {
    if (!user?.id) return; // Skip data load if no user
  ...
  if (loading) return <div>Loading...</div>; // Show loading while checking user
