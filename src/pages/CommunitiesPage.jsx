...
const CommunitiesPage = () => {
  ...
  const handleJoinRequest = async (groupId) => {
    try {
      ...
      // Update the member count in the local state
      setStudyGroups((prev) =>
        prev.map((group) =>
          group.id === groupId ? { 
            ...group, 
            members: response.members || group.members 
          } : group,
        ),
      );
      
      // Show success message if join request was sent
      if (response.status === 'request_sent') {
        setError('Join request sent successfully. You will be notified when approved.');
      }
    } catch (err) {
