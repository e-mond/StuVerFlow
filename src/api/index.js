...
// Automatically attach token if available, excluding login, logout, and public endpoints
api.interceptors.request.use(
  (config) => {
    ...
    const isPublicEndpoint = 
      config.url.startsWith("trending/") || 
      config.url === "questions/hot/" || 
      config.url === "new_users/" ||
      config.url === "search/";
    
    if (token && !isLoginRequest && !isLogoutRequest && !isPublicEndpoint) {
