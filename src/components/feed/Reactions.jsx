...
const Reactions = ({ upvotes, downvotes, questionId, answerId, onVoteUpdate }) => {
  ...
  const handleVote = async (voteType) => {
    ...
    try {
      ...
      if (response.upvotes !== undefined) {
        ...
        setDownvoteCount(response.downvotes || downvoteCount);
        
        // Call the parent callback to update vote counts
        if (onVoteUpdate) {
          onVoteUpdate({
            upvotes: response.upvotes,
            downvotes: response.downvotes || downvoteCount
          });
        }
      }
    } catch (error) {
